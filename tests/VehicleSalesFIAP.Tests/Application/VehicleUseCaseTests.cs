...
public sealed class VehicleUseCaseTests
{
    ...
    [Fact]
    public async Task ListSoldVehiclesReturnsOnlySoldVehiclesOrderedByPrice()
    {
        var repository = new InMemoryVehicleRepository();
        var expensive = Vehicle.RegisterForSale(
            "Brand B",
            "Model B",
            2020,
            "Blue",
            Money.From(90000),
            FixedNow.AddDays(-2));
        var affordable = Vehicle.RegisterForSale(
            "Brand A",
            "Model A",
            2020,
            "Black",
            Money.From(60000),
            FixedNow.AddDays(-2));
        var available = Vehicle.RegisterForSale(
            "Brand C",
            "Model C",
            2020,
            "White",
            Money.From(40000),
            FixedNow);
        expensive.SellTo("buyer-b", FixedNow.AddDays(-1));
        affordable.SellTo("buyer-a", FixedNow.AddDays(-1));
        repository.Items.AddRange([expensive, affordable, available]);
        var useCase = new ListSoldVehiclesUseCase(repository);

        var response = await useCase.HandleAsync();

        Assert.Collection(
            response,
            first => Assert.Equal(60000, first.Price),
            second => Assert.Equal(90000, second.Price));
    }
