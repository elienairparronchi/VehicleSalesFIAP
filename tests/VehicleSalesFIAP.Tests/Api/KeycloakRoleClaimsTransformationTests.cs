...
public sealed class KeycloakRoleClaimsTransformationTests
{
    ...
    public async Task TransformAsyncAddsRealmRolesAsAspNetRoles()
    {
        ...
        var transformation = new KeycloakRoleClaimsTransformation("vehiclesalesfiap-api");
    ...
    [Fact]
    public async Task TransformAsyncReadsOnlyRolesFromConfiguredClient()
    {
        var identity = new ClaimsIdentity(
            [
                new Claim(
                    ...
                    """
                    {
                      "unrelated-client": { "roles": ["buyer"] },
                      "vehiclesalesfiap-api": { "roles": ["vehicle-manager"] }
                    }
                    """)
            ],
            authenticationType: "Bearer");
        ...
        Assert.True(transformed.IsInRole(ApplicationRoles.VehicleManager));
        Assert.False(transformed.IsInRole(ApplicationRoles.Buyer));
    }

    [Fact]
    public async Task TransformAsyncIgnoresMalformedOptionalRoleClaims()
    {
        var identity = new ClaimsIdentity(
            [
