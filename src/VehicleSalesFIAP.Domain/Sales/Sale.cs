...
public sealed class Sale
{
    ...
    internal static Sale Create(Guid vehicleId, string buyerId, Money purchasePrice, DateTimeOffset purchasedAt)
    {
        ...
        {
            throw new DomainException("The purchase price is required.");
        }

        var purchasePriceSnapshot = new Money(purchasePrice.Amount, purchasePrice.Currency);

        return new Sale(Guid.NewGuid(), vehicleId, buyerId, purchasePriceSnapshot, purchasedAt);
    }
